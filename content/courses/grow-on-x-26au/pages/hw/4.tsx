...
const spec: ProjectSpec = {
  ...
  outDate: "Week 5",
  dueDate: "Week 6 (design + first week&rsquo;s data)",
  overview: [
  ...
  background: [
    "Eric Ries&rsquo;s distinction between vanity metrics and actionable metrics (2009) is the foundation of this assignment. Total follower count and total likes are vanity metrics: they go up almost regardless of what you do, and they tell you nothing about whether your specific change caused anything. Impressions per follower and profile-visit-to-follow rate are actionable metrics because they react to specific inputs and reveal whether the audience you already have is amplifying your work. Lenny Rachitsky&rsquo;s framing of the North Star Metric, or NSM (2021), sharpens this: every account at every stage has one metric that, if it moves in the right direction, predicts downstream growth better than anything else. For most accounts in the 0&ndash;1k phase, that NSM is profile-visit-to-follow rate, because it reflects whether new visitors are finding your content compelling enough to commit. Statweestics (2026) documents that accounts with a profile-visit-to-follow rate above 3% grow roughly twice as fast as accounts at or below 1.5%, holding posting frequency constant. Welsh (2023) maps this onto a funnel: impressions drive profile visits, profile visits drive follows, follows drive trust, trust drives revenue. A 30-day experiment that moves one metric in that funnel is the simplest honest test of whether you understand your own account.",
  ],
  instructions: [
    {
      title: "Pick ONE specific hypothesis",
      body: "Format: &ldquo;Doing X for 30 days will move metric Y from baseline B to target T.&rdquo;",
      steps: [
        ...
        "T = your target. State a number and justify it in one sentence. &ldquo;I expect 2.0 because Statweestics (2026) reports that accounts in my niche average 1.9 at this follower count&rdquo; is a justification. &ldquo;I expect it to be higher&rdquo; is not.",
      ],
    },
    {
      title: "Design the comparison",
      body: "Either A/B (alternate days or alternate weeks between control and treatment) or pre-post (baseline = HW3 weeks 1&ndash;2, treatment = the next 14 days). Pre-post is simpler; A/B is cleaner. Both are acceptable.",
      steps: [
        ...
        "Write your stopping rule: &ldquo;I will call this a win if Y reaches T by day 21. If Y has not moved in the target direction by day 21, I will call it flat.&rdquo;",
        "Write your kill switch: the condition under which you abandon the experiment early to protect your account. Example: &ldquo;If impressions per follower drops more than 40% below baseline for three straight days, I stop and revert.&rdquo; The kill switch protects you from running a harmful change all the way to day 30 out of sunk-cost commitment.",
      ],
    },
    {
    ...
    },
    {
      ...
      body: "At the end of day 7, write a structured retrospective of 300 to 500 words covering four things: (1) your signal metric&rsquo;s current value versus your target, (2) one observation that surprised you, (3) one thing you would change about the experiment design now that you have real data, and (4) your explicit decision: continue as designed, adjust the treatment (explain what changes and why), or kill (explain what the data showed and what you would test next). Ries (2009) calls this the &ldquo;pivot or persevere&rdquo; decision. Name it clearly.",
    },
  ],
  deliverables: [
    "Experiment design document: hypothesis in X&rarr;Y from B to T format, comparison structure (A/B or pre-post), stopping rule, and kill switch.",
    ...
    "Week-1 retrospective (300&ndash;500 words): signal metric value versus target, one surprise, one design change you would make, and an explicit continue/adjust/kill decision.",
  ...
  rubric: [
    ...
    { dimension: "Comparison structure is clean (stopping + kill rules)", points: 15, notes: "Both rules are concrete. Vague rules (&ldquo;if it&rsquo;s not working&rdquo;) score 5." },
  ...
  hints: [
    "Ries (2009): a metric is actionable only if a specific action reliably causes it to change. Before you commit to your Y, ask: &ldquo;What exactly would I do differently if this number went up?&rdquo; If you can&rsquo;t answer, pick a different metric.",
    ...
    "Optional: Kallaway walks through a practical growth experiment on X in &ldquo;Twitter Marketing: How To Grow An Audience On X In 2026&rdquo; (~18 min, https://www.youtube.com/watch?v=7hqGOI5wba8). Watch before finalizing your hypothesis design.",
  ],
