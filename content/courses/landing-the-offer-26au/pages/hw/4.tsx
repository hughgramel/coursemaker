...
const spec: ProjectSpec = {
  ...
  instructions: [
    {
      ...
      body: "Choose one product from the list in the overview. Write functional requirements (what the system must do) and non-functional requirements (latency targets, availability SLA, consistency guarantees, scale targets). Keep the combined requirements section under one page. Defining scope here is not busywork&mdash;it is the constraint that makes every downstream decision defensible. A &ldquo;URL shortener&rdquo; that must handle 10 QPS and one that must handle 100,000 QPS are different systems. Say which one you are building.",
      steps: [
        "List 4&ndash;6 functional requirements. Each is a user-facing behavior the system provides.",
        "List 3&ndash;5 non-functional requirements: read/write latency p99, availability (e.g., 99.99%), consistency model, geographic scope.",
        "State one explicit out-of-scope decision and why (e.g., &ldquo;We do not support analytics in this design; that is a separate service.&rdquo;).",
      ],
    },
    {
      title: "Capacity estimate",
      body: "Show the arithmetic. State your DAU assumption, derive QPS from it, and project storage consumed per year and peak bandwidth. Engineers respect numbers that come from stated assumptions, not numbers pulled from nowhere. If you later find a component that can&rsquo;t handle the load you projected, the estimate exposes the bottleneck early.",
      steps: [
    ...
    {
      title: "Data model",
      body: "List the core entities, their primary keys, and the indexes you would create. Then commit to SQL or NoSQL and defend the choice in two or three sentences. &ldquo;NoSQL scales better&rdquo; is not a defense. &ldquo;The access pattern is always a single-key lookup by user ID, the schema will evolve frequently, and we do not need multi-row transactions&rdquo; is.",
      steps: [
    ...
    },
    {
      ...
      steps: [
        "List 2&ndash;3 cache entries: key, value, TTL, and the read pattern that justifies caching it.",
  ...
  deliverables: [
    "A 5&ndash;8 page write-up (PDF or Markdown) covering all seven sections above: requirements, capacity, API, data model, storage, caching, and edge cases.",
    "One Mermaid component diagram embedded in or appended to the write-up.",
    "A brief cover paragraph (3&ndash;5 sentences) naming the product you chose and stating the one design decision you found hardest to make.",
  ],
  rubric: [
    ...
    { dimension: "API design", points: 15, notes: "5&ndash;10 endpoints with request/response shapes and error codes. Idempotency addressed where needed." },
    ...
    { dimension: "Edge cases", points: 10, notes: "3&ndash;5 edge cases that expose real failure modes, not trivial input validation." },
    { dimension: "Polish", points: 5, notes: "Diagram is consistent with prose. Write-up is readable and within the page budget." },
  ],
  hints: [
    "Read Donne Martin&rsquo;s system-design-primer (https://github.com/donnemartin/system-design-primer) before you start. The order-of-operations section is the most important part.",
    ...
    "Hussein Nasser&rsquo;s YouTube channel covers the protocol-level details (why TCP head-of-line blocking matters for video, how Postgres MVCC affects write throughput) that make the difference between a surface-level answer and a real one.",
    "Do NOT optimize before requirements. Every component choice must trace back to a constraint you stated in section one.",
    "The order of operations matters more than any individual component. A candidate who does requirements, then capacity, then API is recognizably prepared. A candidate who jumps to &ldquo;we&rsquo;d use a microservices architecture&rdquo; before stating a single requirement has already lost points.",
