...
const spec: ProjectSpec = {
  ...
  goals: [
    ...
    "Write a clear, non-jargon justification of your typeface and color decisions that ties each choice to the product&rsquo;s intended tone.",
  ],
  background: [
    ...
    "Auto-layout turns a frame into a smart container that stacks its children horizontally or vertically and adjusts its own size as content changes. The key settings are direction (horizontal or vertical), spacing between items, and padding. When you set a child&rsquo;s width to &ldquo;Fill container,&rdquo; it stretches to fill available space; when you set it to &ldquo;Hug contents,&rdquo; it shrinks to fit. These two settings together handle almost every layout problem you will encounter this week.",
  ...
  instructions: [
    {
      ...
      body: "Create a new Figma file named &ldquo;[Your product] Starter Kit.&rdquo; Add three pages: Components, Playground, and Cover. All main components live on Components. Playground is where you test assembled screens later. Cover is a single frame you will use as the file thumbnail. At the top of the Components page, create a frame labeled &ldquo;Styles&rdquo; and define at minimum: two text styles (a heading and a body style), a primary fill color, a neutral fill color, and a surface color. Name every style. Do not use raw hex values anywhere in the components you build.",
      steps: [
    ...
    },
    {
      ...
      steps: [
        ...
        "Choose a typeface pair or single typeface that matches the product&rsquo;s intended tone. Justify this choice in your written deliverable.",
    ...
    },
    {
      ...
      body: "Build each component in order on the Components page, using auto-layout for every frame. Each component must use only named styles, no raw hex values. Where variants are specified, connect them using the Variants panel so they live in a single component set. Group component sets into labeled sections on the page using Figma&rsquo;s section tool. The section labels should match the component names below.",
      steps: [
        ...
        "Hero block: two variants (With image, Text only). A full-width section frame with a heading, a subheading, and a CTA button. The &ldquo;With image&rdquo; variant adds an image placeholder to the right.",
    ...
    },
    {
      ...
      steps: [
        ...
        "Layers are named. No &ldquo;Frame 47&rdquo; or &ldquo;Rectangle 12&rdquo; in any published component.",
        "Component names in the sidebar follow a consistent convention, for example Component / Variant.",
      ],
    },
  ],
  deliverables: [
    "A Figma share link with &ldquo;Anyone with the link can view&rdquo; enabled. The link must open to the Components page.",
  ...
  hints: [
    "Use the Iconify plugin (search for it in the Figma community) to pull in any icon as an SVG frame. Drop the icon into a component as a named layer called &ldquo;icon&rdquo; and expose it as a boolean property so consumers can toggle it off without breaking the layout.",
    ...
    "Set your component frame to &ldquo;Hug contents&rdquo; in both axes during construction. This makes it easy to see whether your auto-layout is working correctly. Switch to fixed widths only for full-width components like Navbar and Footer.",
    "Give every layer a meaningful name before you turn a frame into a component. Layer names become the property labels that instance users see. &ldquo;Label&rdquo; and &ldquo;Icon&rdquo; are better than &ldquo;Text 7&rdquo; and &ldquo;Vector 3.&rdquo;",
    "The 200-word justification is graded on specificity, not length. &ldquo;I chose Inter because it is clean&rdquo; scores poorly. &ldquo;I chose Inter because its narrow letterforms keep labels readable at 12px inside the badge and input components, and its lack of decorative serifs matches the task-focused tone of a project-management tool&rdquo; scores well.",
  ],
