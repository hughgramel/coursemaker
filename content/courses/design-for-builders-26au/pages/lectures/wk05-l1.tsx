...
const spec: LectureSpec = {
  ...
  followups: [
    "Open the Tailwind CSS spacing documentation. Find the values for p-4, p-8, and p-16. Convert each to pixels (1 unit = 4px). Which values land on the 8-point grid? Which ones are 4px-grid steps only? What does that tell you about Tailwind’s relationship to the 8-point grid?",
    "The Spec Network (2016) article contrasts “hard grid” and “soft grid” approaches. In your own words, describe the difference. When would you prefer the soft grid over the hard grid? Give a specific UI component where each approach makes more sense.",
    ...
    "A designer sets a button’s inner padding to 12px top/bottom and 20px left/right. Neither value is on the 8-point grid. Using the distinction between inner padding and outer margin from the lecture, explain why this choice causes problems when you place the button inside a layout that uses the 8-point grid.",
  ],
};
